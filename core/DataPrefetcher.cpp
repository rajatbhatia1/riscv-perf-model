...
namespace olympia
{
    ...
    DataPrefetcher::DataPrefetcher(sparta::TreeNode* node, const DataPrefetcherParameterSet* p) :
        ...
        ev_gen_prefetch_{getEventSet(), "gen_prefetch_event",
                         CREATE_SPARTA_HANDLER(DataPrefetcher, generatePrefetch_)},
        ev_handle_incoming_req_{getEventSet(), "handle_incoming_event",
                                CREATE_SPARTA_HANDLER(DataPrefetcher, handleIncomingReq_)},
        ...
        req_queue_credits_out_{getPortSet(), "out_req_queue_credit"}
    {
        ...
        prefetcher_queue_credits_in_.registerConsumerHandler(CREATE_SPARTA_HANDLER_WITH_DATA(
            DataPrefetcher, receivePrefetchQueueCredits_, uint32_t));
